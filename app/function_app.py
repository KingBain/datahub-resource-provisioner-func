...
def get_sync_func_mappings():
    ...
    mappings_pre = {f"terraform:{key}": value for key, value in mappings.items()}
...
def new_sync_workspace(workspace_definition: dict[str, Any]) -> None:
    ...
    workspace_name = workspace_definition["Workspace"]["Acronym"]
    logger.info("Synchronizing workspace users for %s", workspace_name)
    ...
    if not workspace_templates:
        ...
        logger.info("Got template names: %s", template_names)

        for t in template_names:
            if t not in sync_mappings:
                logger.info("Skipping template %s", t)
                continue

            name, sync_fn = sync_mappings[t]
            logger.debug("name: %s, func: %s", name, sync_fn.__name__)

            try:
                logger.info("Synchronizing %s for %s.", name, workspace_name)
    ...
    if health_status == hcm.HealthcheckMessage.STATUS_HEALTHY:
        logger.info("Successfully synced workspace %s", workspace_name)
    else:
...
def sync_databricks_workspace_users_function(
    ...
    """
    databricks_host = workspace_definition["AppData"]["DatabricksHostUrl"]
    environment_name = os.environ["DataHub_ENVNAME"]
    subscription_id = os.environ["AzureSubscriptionId"]

    workspace_client = dtb_utils.get_workspace_client(databricks_host)
