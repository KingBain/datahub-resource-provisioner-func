...
def _is_managed_blob_role(role_definition_id):
    ...
    return role_id in (READER, CONTRIBUTOR)
...
def remove_existing_role(client: AuthorizationManagementClient, scope, user_object_id):
    ...
    for role in role_assignments:
        if role.principal_id == user_object_id and _is_managed_blob_role(
...
def get_blob_container_scope(
    subscription_id, resource_group_name, storage_account_name, container_name
):
    """Build the Azure resource scope for a blob container."""
    return (
        f"{get_scope(subscription_id, resource_group_name, storage_account_name)}"
...
def get_blob_container_scopes(
    ...
    if not blob_containers:
        raise ValueError(
            "No blob containers found in workspace definition for storage role synchronization"
        )

    return [
        get_blob_container_scope(
            subscription_id, resource_group_name, storage_account_name, container_name
        )
...
def check_blob_reader_role(
    client: AuthorizationManagementClient, scope, user_object_id
):
...
def synchronize_access_policies(
    ...
    scopes = get_blob_container_scopes(
        subscription_id, rg_name, sg_account, blob_containers
    )
            ...
            for scope in scopes:
                ...
                if user["Role"] == "Guest":
                    if reader_role is None:
                        logger.info(
                            "Assigning user %s to access policies - read-only", user_id
                        )
                        ...
                        logger.info(
                            "User %s already has read-only access policies", user_id
                        )
                else:
                    if reader_role is None:
                        logger.info(
                            "Assigning user %s to access policies - read-write", user_id
                        )
                        ...
                        logger.info(
                            "User %s already has read-write access policies", user_id
                        )
    ...
    for user in (
        ...
        try:
            ...
            logger.exception(
                "Error processing user %s access policies", user["ObjectId"]
            )
